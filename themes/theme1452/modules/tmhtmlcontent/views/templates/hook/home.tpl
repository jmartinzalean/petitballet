...
{if isset($htmlitems) && $htmlitems}
		<div id="tmhtmlcontent_{$hook|escape:'htmlall':'UTF-8'}" class="block">
				<h4 class='title_block'><span>{l s='Popular Categories' mod='tmhtmlcontent'}</span></h4>
						...
						{foreach name=items from=$htmlitems item=hItem}
								{if $hook == 'left' || $hook == 'right'}
										<li class="tmhtmlcontent-item-{$smarty.foreach.items.iteration|escape:'htmlall':'UTF-8'} col-xs-12 {$hItem.specific_class|escape:'htmlall':'UTF-8'}">
												{else}
										<li class="tmhtmlcontent-item-{$smarty.foreach.items.iteration|escape:'htmlall':'UTF-8'} col-xs-3 {$hItem.specific_class|escape:'htmlall':'UTF-8'}">
								{/if}
						{if $hItem.url}
