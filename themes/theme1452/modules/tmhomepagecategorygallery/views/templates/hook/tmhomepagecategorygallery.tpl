				...
				<ul id="tmhomepagecategorygallery" class="tmhomepagecategorygallery">
						{foreach from=$items item=item name=slide}
								<li id="tmhomepagecategorygallery-item-{$smarty.foreach.slide.iteration|escape:'htmlall':'UTF-8'}" class="item-{$item.id_item|escape:'htmlall':'UTF-8'} item{if isset($item.specific_class) && $item.specific_class} {$item.specific_class|escape:'htmlall':'UTF-8'}{/if}">
										{strip}
												<a class="b-lazy" href="{$link->getCategoryLink($item.category->id_category|escape:'htmlall':'UTF-8')}" title="{$item.category->name|escape:'htmlall':'UTF-8'}" data-src="
																{if $item.category->id_image}
																		{$link->getCatImageLink($item.category->link_rewrite, $item.category->id_image, 'category_default')|escape:'htmlall':'UTF-8'}
																{else}
																		{$link->getCatImageLink($item.category->link_rewrite, $lang_iso, 'default-category_default')|escape:'htmlall':'UTF-8'}
																{/if}">
												</a>
										{/strip}
										{if $item.display_name || $item.display_description || $item.button}
												<div class="tmhomepagecategorygallery-content">
														...
														{/if}
														{if $item.button}
																<a class="tmhomepagecategorygallery-button btn btn-md btn-secondary-2 icon-right" href="{$link->getCategoryLink($item.category->id_category|escape:'htmlall':'UTF-8')}" title="{$item.category->name|escape:'htmlall':'UTF-8'}"><span>{l s='Shop now' mod='tmhomepagecategorygallery'}</span></a>
														{/if}
												</div>
										{/if}
								</li>
				...
				{if isset($display_gallery) && $display_gallery}
						{if count($items) > 1}
								<ul id="tmhomepagecategorygallery-nav" class="tmhomepagecategorygallery-nav">
										{foreach from=$items item=item name=slide}
												<li class="tmhomepagecategorygallery-nav-item-{$smarty.foreach.slide.iteration|escape:'htmlall':'UTF-8'}">
														<a href="#tmhomepagecategorygallery-item-{$smarty.foreach.slide.iteration|escape:'htmlall':'UTF-8'}">{$smarty.foreach.slide.iteration|escape:'htmlall':'UTF-8'}</a>
												</li>
										{/foreach}
								</ul>
						{/if}
