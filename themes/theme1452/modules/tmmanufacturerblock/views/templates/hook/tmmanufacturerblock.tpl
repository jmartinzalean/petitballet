...
{if $manufacturers}
		<div id="tm_manufacturers_block" class="clearfix block">
				<h4 class='title_block'><span>{l s='Brands' mod='tmmanufacturerblock'}</span></h4>
												...
												{if isset($display_image) && $display_image}
														<a href="{$link->getmanufacturerLink($manufacturer.id_manufacturer, $manufacturer.link_rewrite)|escape:'html':'UTF-8'}" title="{l s='More about %s' sprintf=[$manufacturer.name] mod='tmmanufacturerblock'}">
																<img class="img-responsive b-lazy"
																					...
																					data-src="{$img_manu_dir|escape:'html':'UTF-8'}{$manufacturer.image|escape:'html':'UTF-8'}-{$image_type|escape:'html':'UTF-8'}.jpg" alt="{$manufacturer.image|escape:'html':'UTF-8'}"/>
														</a>
												{/if}
												{if isset($display_name) && $display_name}
														<a class="manufacturer-name" href="{$link->getmanufacturerLink($manufacturer.id_manufacturer, $manufacturer.link_rewrite)|escape:'html'}" title="{l s='More about %s' sprintf=[$manufacturer.name] mod='tmmanufacturerblock'}">{$manufacturer.name|escape:'html':'UTF-8'}</a>
												{/if}
												{$manufacturer.short_description|truncate:80:'...'}
										</li>
								{/if}
						{/foreach}
				</ul>
