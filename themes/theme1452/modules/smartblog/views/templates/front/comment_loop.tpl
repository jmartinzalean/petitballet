      ...
      <div class="comment-content clearfix">
        ...
        <div class="created">
										<span itemprop="commentTime">
												{$childcommnets.created|date_format:"%d %B %Y"}
										</span>
        </div>
        ...
        {if Configuration::get('smartenablecomment') == 1}
          {if $comment_status == 1}
            <div class="reply">
              <a onclick="return addComment.moveForm('comment-{$comment.id_smart_blog_comment}', '{$comment.id_smart_blog_comment}', 'respond', '{$smarty.get.id_post}')" class="comment-reply-link btn btn-default btn-md icon-right"><span>{l s='Reply' mod='smartblog'}</span></a>
            </div>
          {/if}
        {/if}
      </div>
