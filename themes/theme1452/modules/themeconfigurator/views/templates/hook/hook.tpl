{if isset($htmlitems) && $htmlitems}
  ...
  <div id="htmlcontent_{$hookName}">
    <ul class="htmlcontent-{$hookName} clearfix row">
      {foreach name=items from=$htmlitems item=hItem}
        <li class="htmlcontent-item-{$smarty.foreach.items.iteration|escape:'htmlall':'UTF-8'}{if  $hookName =='home'} col-xs-4{elseif $hookName =='footer'} col-xs-4{elseif $hookName == 'top'} {/if}">
          {if $hItem.url}
