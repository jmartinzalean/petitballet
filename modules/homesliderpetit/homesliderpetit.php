...
include_once(_PS_MODULE_DIR_.'homesliderpetit/HomeSlidePetit.php');

class HomeSliderPetit extends Module
{
	protected $_html = '';
	...
	protected function createTables()
	{
		...
		$res = (bool)Db::getInstance()->execute('
			CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'homesliderpetit` (
		...
		$res &= Db::getInstance()->execute('
			CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'homesliderpetit_slides` (
			  `id_homeslider_slides` int(10) unsigned NOT NULL AUTO_INCREMENT,
		...
		$res &= Db::getInstance()->execute('
			CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'homesliderpetit_slides_lang` (
	...
	protected function deleteTables()
	{
		...
		{
			$to_del = new HomeSlidePetit($slide['id_slide']);
			$to_del->delete();
		}

		return Db::getInstance()->execute('
			DROP TABLE IF EXISTS `'._DB_PREFIX_.'homesliderpetit`, `'._DB_PREFIX_.'homesliderpetit_slides`, `'._DB_PREFIX_.'homesliderpetit_slides_lang`;
		');
	}
	...
	public function getContent()
	{
		...
		elseif (Tools::isSubmit('addSlide') || (Tools::isSubmit('id_slide') && $this->slideExists((int)Tools::getValue('id_slide'))))
		{
			...
			{
				$associated_shop_ids = HomeSlidePetit::getAssociatedIdsShop((int)Tools::getValue('id_slide'));
	...
	protected function _postProcess()
	{
		...
		{
			$slide = new HomeSlidePetit((int)Tools::getValue('id_slide'));
			if ($slide->active == 0)
				$slide->active = 1;
			else
				$slide->active = 0;
		...
		{
			$slide = new HomeSlidePetit((int)Tools::getValue('delete_id_slide'));
			$res = $slide->delete();
			$this->clearCache();
			if (!$res)
	...
	public function hookActionShopDataDuplication($params)
	{
		Db::getInstance()->execute('
			INSERT IGNORE INTO '._DB_PREFIX_.'homesliderpetit (id_homeslider_slides, id_shop)
			SELECT id_homeslider_slides, '.(int)$params['new_id_shop'].'
			FROM '._DB_PREFIX_.'homesliderpetit
	...
	public function getNextPosition()
	{
		$row = Db::getInstance(_PS_USE_SQL_SLAVE_)->getRow('
			...
			FROM `'._DB_PREFIX_.'homesliderpetit_slides` hss, `'._DB_PREFIX_.'homesliderpetit` hs
	...
	public function getSlides($active = null)
	{
		...
		return Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			...
			FROM '._DB_PREFIX_.'homesliderpetit hs
			LEFT JOIN '._DB_PREFIX_.'homesliderpetit_slides hss ON (hs.id_homeslider_slides = hss.id_homeslider_slides)
			LEFT JOIN '._DB_PREFIX_.'homesliderpetit_slides_lang hssl ON (hss.id_homeslider_slides = hssl.id_homeslider_slides)
	...
	public function renderList()
	{
		...
		foreach ($slides as $key => $slide)
		{
			...
			$associated_shop_ids = HomeSlidePetit::getAssociatedIdsShop((int)$slide['id_slide']);
