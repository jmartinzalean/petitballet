      ...
      <div class="block_content">
        <form action="#" id="layered_form">
          <div>
                            ...
                            {foreach from=$filter.values key=id_value item=value}
                              {if $id_value == $filter_key && !is_numeric($filter_value) && ($filter.type eq 'id_attribute_group' || $filter.type eq 'id_feature') || $id_value == $filter_value && $filter.type neq 'id_attribute_group' && $filter.type neq 'id_feature'}
                                <li>
                                  ...
                                  {$filter.name} : <span>{$value.name}</span>
                                </li>
                              {/if}
                            {/foreach}
                          {/if}
            ...
            {/if}
            {foreach from=$filters item=filter}
              {if isset($filter.values) && (($filter.type == 'price' && !$PS_CATALOG_MODE) || $filter.type != 'price')}
                {if isset($filter.slider)}
                ...
                <div class="layered_subtitle_heading title-collapse">
                  <a data-toggle="collapse" data-parent="#layered_block_left" href="#ul_layered_{$filter.type}_{$filter.id_key}" class="layered_subtitle">{$filter.name|escape:'html':'UTF-8'}</a>
                </div>
                <ul id="ul_layered_{$filter.type}_{$filter.id_key}" class="panel-collapse collapse col-lg-12 layered_filter_ul{if isset($filter.is_color_group) && $filter.is_color_group} color-group{/if}">
                  {if !isset($filter.slider)}
                        ...
                        {if $value.nbr || !$hide_0_values}
                          <li class="nomargin {if $smarty.foreach.fe.index >= $filter.filter_show_limit}hiddable{/if} col-lg-12">
                            {if isset($filter.is_color_group) && $filter.is_color_group}
                              <input class="color-option{if isset($value.color) && ($value.color == '#ffffff' || $value.color == 'white')} white-color{/if} {if isset($value.checked) && $value.checked}on{/if} {if !$value.nbr}disable{/if}" type="button" name="layered_{$filter.type_lite}_{$id_value}" data-rel="{$id_value}_{$filter.id_key}" id="layered_id_attribute_group_{$id_value}" {if !$value.nbr}disabled="disabled"{/if} style="color:{if isset($value.color)}{$value.color}{/if} ;background: {if isset($value.color)}{if file_exists($smarty.const._PS_ROOT_DIR_|cat:"/img/co/$id_value.jpg")}url(img/co/{$id_value}.jpg){else}{$value.color}{/if}{else}#CCC{/if};" />
