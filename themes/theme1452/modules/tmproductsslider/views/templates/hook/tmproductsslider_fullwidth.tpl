...
<script type="text/javascript">
	jQuery(document).ready(function ($) {
		var options = {
			$AutoPlay: {if ($settings.fullwidth_extended_settings && $settings.fullwidth_slider_autoplay)}true{else}false{/if},                //[Optional] Whether to auto play, to enable slideshow, this option must be set to true, default value is false
...
{if isset($slides) && $slides}
	<div id="tm-products-slider" class="{$settings.slider_type|escape:'htmlall':'UTF-8'}">
		<div id="slider_container" style="width: {$settings.fullwidth_slider_width|escape:'htmlall':'UTF-8'}px;">
			<div data-u="slides" class="main-slides" style="width: {$settings.fullwidth_slider_width|escape:'htmlall':'UTF-8'}px; height: {$settings.fullwidth_slider_height|escape:'htmlall':'UTF-8'}px;">
				{foreach from=$slides item=slide}
					<div class="slide-inner">
						<div data-u="thumb">
							{if isset($slide.image) && $slide.image}
								<img src="{$link->getImageLink($slide.info->name, $slide.image.id_image, 'medium_default')|escape:'htmlall':'UTF-8'}" alt="{$settings.product_name}"/>
							{/if}
						...
						<div class="sliders-wrap">
							<div class="slide-info {if $slide.image}col-xs-4{else}col-xs-12{/if}">
								...
								{if $settings.product_name}<h3>
									<a href="{$slide.info->getLink()|escape:'htmlall':'UTF-8'}">{$slide.info->name|truncate:20:' '|escape:'htmlall':'UTF-8'}</a>
									</h3>{/if}
									{if $slide.info->price && $slide.info->show_price && $settings.prices && !isset($restricted_country_mode) && !$PS_CATALOG_MODE}
                  ...
                  {if $slide.info->specificPrice.reduction_type == 'percentage'}
									...
									<div class="buttons-container">
										{if $slide.info->available_for_order && !isset($restricted_country_mode) && !$PS_CATALOG_MODE && $settings.cart_button}
											{if (!isset($slide.info->customization_required) || !$slide.info->customization_required) && $slide.info->quantity > 0}
												<a class="ajax_add_to_cart_button btn btn-md btn-default cart-button icon-right" href="{$link->getPageLink('cart', true, NULL, "qty=1&amp;id_product={$slide.info->id|intval}&amp;token={$static_token}&amp;add", false)|escape:'html':'UTF-8'}" rel="nofollow" title="{l s='Add to cart' mod='tmproductsslider'}" data-id-product="{$slide.info->id|intval}" data-minimal_quantity="{$slide.info->minimal_quantity|intval}">
													<span>{l s='Add to cart' mod='tmproductsslider'}</span>
												</a>
											{else}
												<a href="{$slide.info->getLink()|escape:'htmlall':'UTF-8'}" class="btn btn-md btn-default cart-button">
													<span>{l s='Add to cart' mod='tmproductsslider'}</span>
												</a>
											{/if}
										{/if}
										{if $settings.more_button}
											<a href="{$slide.info->getLink()|escape:'htmlall':'UTF-8'}" class="btn lnk_view btn btn-default">
							...
							{if isset($slide.image) && $slide.image}
								<div class="slide-image col-xs-8">
												...
												<div id="inner-slider-{$slide.info->id|escape:'htmlall':'UTF-8'}" class="sliders-inner">
													<div data-u="slides" class="inner-slides">
														{foreach from=$slide.images item=img}
															<div>
																<img data-u="image" class="img-responsive b-lazy-product-slider"
																...
																<img data-u="thumb" class="b-lazy-product-slider"
																					src="{$img_dir}product-lazy-placeholder.jpg"
																					data-src="{$link->getImageLink($slide.info->name, $img.id_image, 'small_default')|escape:'htmlall':'UTF-8'}" alt="{$slide.info->name|escape:'htmlall':'UTF-8'}"/>
															</div>
														{/foreach}
													</div>
												  {if $settings.fullwidth_extended_settings}
			...
			{/if}
			{if $settings.fullwidth_extended_settings && $settings.fullwidth_slider_pagination}
			...
			{/if}
			{if ($settings.fullwidth_extended_settings && $settings.fullwidth_slider_thumbnails)}
