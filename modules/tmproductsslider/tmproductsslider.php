...
class TMProductsSlider extends Module
{
    ...
    {
        if (!$slides = TMProductSlide::getShopSlides($this->context->shop->id, $this->context->language->id)) {
            $slides = array();
        }
    ...
    public function prepareNewTab()
    {
        ...
        $this->context->smarty->assign(
            'is_slide',
            TMProductSlide::checkSlideExist((int)Tools::getValue('id_product'), $this->context->shop->id)
        );
    ...
    protected function addSlide($id_product)
    {
        ...
        foreach ($shops as $id_shop) {
            if (!TMProductSlide::checkSlideExist($id_product, $id_shop)) {
    ...
    {
        if ($slide_id = TMProductSlide::checkSlideExist($id_product, $this->context->shop->id)) {
    ...
    {
        $slides = array();
        $shopslides = TMProductSlide::getShopSlides($this->context->shop->id, $this->context->language->id);
    ...
    public function hookDisplayTopColumn()
    {
        ...
        return $this->display(__FILE__, 'tmproductsslider_'.$slidersettings['slider_type'].'.tpl');
    }
