...
$(document).ready(function () {
  if (typeof(carousel_status) != 'undefined' && carousel_status && $('#homepage-carousel').length > 0) {
    if (TM_PLG_TYPE !== 'slideshow') {
      ...
      homeTabSlickInit('#homepage-carousel');

      $('#home-page-tabs li').on('click.initSlick', function (e) {
        e.preventDefault();

        $('#homepage-carousel').slick('unslick');
        thisClass = $(this).children('a').attr('class');
        content = $('.tab-content ul.' + thisClass).html();
        $('#homepage-carousel').empty();
        $('#homepage-carousel').append(content);
      ...
      $(window).load(function () {
        setTimeout(function () {
          $('#home-page-tabs li:first-child').trigger('click.initSlick');
        }, 500);
      });
    }
  }
...
function homeTabSlickInit(elem) {
  $(elem).slick({
    slidesToShow: 4,
    slidesToScroll: 2,
    draggable: false,
